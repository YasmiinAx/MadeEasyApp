...
export default function CalendarScreen({ navigation }) {
  ...
  // Merge completed status from taskList
  const displayedTasks = calendarTasks
    .filter(task => task.date === selected)
    .map(task => {
      // Find matching task in taskList by name (original task)
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.card, { flex: 1 }]}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={{ color: "#bbb", marginTop: 10 }}>No tasks scheduled</Text>}
        />
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  header: { paddingTop: 30, color: '#FFFFFF', fontSize: 25 },
  addButton: { marginTop: 30, backgroundColor: "#E891D6", paddingHorizontal: 15, paddingVertical: 5, borderRadius: 8 },
  addButtonText: { color: "#fff", fontWeight: "600", fontSize: 16 },
  calendar: { borderRadius: 16, borderWidth: 1, borderColor: '#3D2938', marginBottom: 20, overflow: "hidden" },
  subHeader: { color: "#fff", fontSize: 22 },
  taskCard: { backgroundColor: "#2D1F2B", paddingTop: 10, borderRadius: 14 },
  taskTitle: { color: "#fff", fontSize: 16, fontWeight: "600" },
  taskRow: { backgroundColor: '#271925', padding: 15, paddingHorizontal: 20, borderRadius: 10, flexDirection: 'row', alignItems: 'center' },
  priorityCircle: { width: 10, height: 10, borderRadius: 6, marginRight: 10 },
  taskTime: { color: '#A894A3', fontSize: 14, marginTop: 2 },
});
